...
function isCreditCardNumber(ccn) {
  const val = ccn.toString().split('');
  let res = [];
  if (val.length % 2 === 0) {
    res = val.map((el, i) => {
      if (i % 2 !== 0) return parseInt(el, 10);
  ...
  const number = res.reduce((count, el) => count + el) % 10 === 0 ? !!1 : !!0;
  return number;
}
...
function getDigitalRoot(num) {
  let val = num.toString()
    .split('')
    .map((el) => +el)
    .reduce((count, el) => count + el);
  if (val > 9) {
    while (val > 9) {
      val = val.toString().split('').reduce((count, el) => +count + +el);
    }
  }
  return val;
}
